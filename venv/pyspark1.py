...
sp_num = sp.select(sp.full_sq.cast("int"), sp.life_sq.cast("int"), sp.price_doc.cast("float"), \
...
test_num = testTable.select(testTable.full_sq.cast("int"), testTable.life_sq.cast("int"), \
...
ttestLFDF = model.transform(test_num).select("features")
#testLFDF.take(10)
...
#This is broken, we need an AWS instance or something
gbt_tTestP = gbt_tTestP.select(gbt_tTestP.prediction)
gbt_tTestP.write.format("com.databricks.spark.csv").save("home/test.csv")
#gbt_tTestP.write.option('path', 'YOUR_S2_PATH').saveAsTable('example_data')
#gbt_tTestP.rdd.to_csv('hello.csv', sep=',', header=True, index=False)
